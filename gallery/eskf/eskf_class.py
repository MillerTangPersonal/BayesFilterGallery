...
from eskf_util import cross, zeta, computeG_grad
...
class ESKF:
    ...
    def predict(self, imu, dt, imu_check, k):
        ...
        if imu_check:
            ...
            # Jacobian matrix
            Fx = np.block([
                [np.eye(3),         dt*np.eye(3),      -0.5*dt**2*self.R.dot(cross(f_k))],
                [np.zeros((3,3)),   np.eye(3),         -dt*self.R.dot(cross(f_k))       ],
    ...
    def UWB_correct(self, uwb, anchor_position, k):
        ...
        # G is 1 x 9
        G = computeG_grad(an_A, an_B, self.t_uv, self.Xpr[k,0:3], self.q_list[k])
