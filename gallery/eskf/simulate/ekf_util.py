...
def getAzEl_Angle(vector, C_BI):
    # param: vector in body frame, 
    #        rotation matrix from inertial to body frame
    # rotate the vector (Vicon frame) into anchor frame
    RAD_TO_DEGREE = 180.0/np.pi
    v_body = C_BI.dot(vector.reshape(-1,1))
    v_body = np.squeeze(v_body)
    azimuth = math.atan2(v_body[1], v_body[0])* RAD_TO_DEGREE
    
    # math.atan2(y, x) --> The result is between -pi and pi
    
    # v_project = np.array([v_body[0], v_body[1], 0])
    # normator = v_body[0]**2 + v_body[1]**2
    # denormator = np.linalg.norm(v_body) * np.linalg.norm(v_project)   
    
    elevation = math.asin(v_body[2] / math.sqrt(v_body[0]**2 + v_body[1]**2 +v_body[2]**2))* RAD_TO_DEGREE
