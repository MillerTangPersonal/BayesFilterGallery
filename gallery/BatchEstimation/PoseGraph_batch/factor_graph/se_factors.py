...
import numpy as np
import math
from se_utils import skew, getTrans, getTrans_in, circle
...
class SE3PriorFactor(Factor):
    ...
    def get_error(self, param_blocks, residuals):
        ...
        residuals[:] = jxl.SE3.log(self.data_prior @ T_op0.inverse())        # log(T_check0 @ T_op0.inverse())
        return True
...
class SE3BetweenFactorTwist(Factor):
    ...
    def get_error(self, param_blocks, residuals):
        ...
        zeta = np.block([
            [phi_skew, rho],
            [0,  0,  0,  0]])
        Psi = np.linalg.expm(self.dt*zeta)
        ...
        residuals[:] = np.squeeze(np.concatenate(rho_now, phi_now), axis=0)
...
class StereoFactor(Factor):
    ...
    def get_error(self, param_blocks, residuals):
        ...
        # pixel = y_k_j
        residuals[:] = self.pixels - np.squeeze(y_o)
